...
const {session}=await createAgentSession({cwd:job.dir,agentDir:path.join(job.dir,'pi'),authStorage,modelRegistry,model:modelRegistry.find('motion-deepseek',modelId),thinkingLevel:'off',resourceLoader:loader,settingsManager,sessionManager:SessionManager.inMemory(job.dir),tools:['read_effect','write_effect','render_preview'],customTools:[
  ...
  {name:'render_preview',label:'验证预览',description:'实际渲染6秒预览，返回成功或错误；失败后修改代码重试',parameters:Type.Object({}),execute:async()=>{
    ...
    try{await renderEffectVideo({template:{compiled,assets:job.assets,backgroundColor},output:path.join(job.dir,'candidate.mp4')});renderedCode=compiled;return result('试渲染成功，动效已可以预览。');}catch(error){throw new Error(String(error.message).slice(0,2000));}
  }}
]});
const events=[];
session.subscribe(event=>{
  if(event.type==='tool_execution_start'||event.type==='tool_execution_end') events.push({time:new Date().toISOString(),event:event.type,tool:event.toolName,...(event.type==='tool_execution_end'?{isError:event.isError}:{})});
...
}finally{fs.writeFileSync(path.join(job.dir,'agent-evidence.json'),JSON.stringify({sdk:'@mariozechner/pi-coding-agent',version:'0.73.1',sessionId:session.sessionId,model:modelId,events,renderVerified:Boolean(renderedCode&&renderedCode===compiled)},null,2),{mode:0o600});session.dispose();}
