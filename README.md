...
动效库包含两个所有账号可选用的内置固定模板：「绿金文字总结」和「绿金流程图解」。图解支持两至三个节点及流程、分支、汇总、公式四种结构，文案与结构随每段总结保存。固定模板直接复用代码渲染，不调用 Pi/DeepSeek 重新编写；个人创作区仍按账号隔离。参考片拆解、模板源码和重建方式见 [内置绿金动效模板](docs/内置绿金动效模板.md)。
